...
MAX_CMD_LENGTH: Final = 253  # 253 = 255 - 1 byte for header - 1 byte for trailer
...
# ----------------------------------
#
LOGIN_CMD: Final = r"LIN"  # login command
LOGIN_RESP: Final = r"^OK"
LOGOUT_CMD: Final = r"LOUT"  # logout command
LOGOUT_RESP: Final = r"^OK"
RID_CMD: Final = r"RID"  # query for the manufacturer's serial number
RID_RESP: Final = r"^RID,(\d+)$"  # e.g. '~RID,123456;\r\n'
VERSION_CMD: Final = r"SP,NR=9"  # query for the software version of the heat pump
VERSION_RESP: Final = r"^SP,NR=9,.*NAME=([^,]+).*VAL=([^,]+).*$"  # e.g. 'SP,NR=9,ID=9,NAME=3.0.20,...,VAL=2321,...'
CLK_CMD: Final = (
...
ALC_CMD: Final = r"ALC"  # query for the last fault message of the heat pump
ALC_RESP: Final = (
...
ALS_CMD: Final = r"ALS"  # query for the fault list size of the heat pump
ALS_RESP: Final = r"^SUM=(\d+)$"  # e.g. 'SUM=2757'
AR_CMD: Final = r"AR"  # query for specific entries of the fault list
AR_RESP: Final = (
    r"^AA,(\d+),(\d+)"  # fault list index and error code (?)
    r",(3[0-1]|[1-2]\d|0[1-9])\.(1[0-2]|0[1-9])\.(\d\d)"  # date, e.g. '14.09.14'
    r"-([0-1]\d|2[0-3]):([0-5]\d):([0-5]\d)"  # time, e.g. '11:52:08'
...
MR_CMD: Final = r"MR"  # fast query for several MP data point values
MR_RESP: Final = r"^MA,(\d+),([^,]+),(\d+)$"  # MP data point number, value and ?; e.g. 'MA,0,-3.4,17'
PRL_CMD: Final = r"PRL"  # query for the time programs of the heat pump
PRL_RESP: Final = (
    r"^SUM=(\d+)$",  # e.g. 'SUM=5'
    r"^PRI{:d},.*NAME=([^,]+).*EAD=([^,]+).*NOS=([^,]+).*STE=([^,]+).*NOD=([^,]+).*$",
)  # e.g. 'PRI0,...'
PRI_CMD: Final = r"PRI{:d}"  # query for a specific time program of the heat pump
PRI_RESP: Final = (
    r"^PRI{:d},.*NAME=([^,]+).*EAD=([^,]+).*NOS=([^,]+).*STE=([^,]+).*NOD=([^,]+).*$"  # e.g. 'PRI2,NAME=..'
)
PRD_CMD: Final = r"PRD{:d}"  # query for the entries of a specific time program of the heat pump
PRD_RESP: Final = (
